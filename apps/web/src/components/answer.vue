...
const reasoningHtml = computed(() => {
  ...
  parent.querySelectorAll('a').forEach(tag => {
    const citationNumber = tag.getAttribute('href');
    if (tag.innerText !== 'citation') return;
...
function processAnswer(answer?: string) {
  ...
  const html = marked.parse(citation, { async: false });
  const parent = document.createElement('div');
  parent.innerHTML = html as string;
...
<template>
  <div class="h-auto w-full text-base leading-7 text-zinc-600 dark:text-gray-200">
    <t-skeleton theme="paragraph" animation="flashed" :loading="!answer && !reasoning" />
          ...
          <template #header>
            <div class="flex flex-nowrap items-center justify-between gap-1">
              <div class="flex flex-nowrap items-center gap-1">
                ...
                <div class="text-xs font-bold text-zinc-600">{{ t('reasoning') }}</div>
              </div>
            </div>
          </template>
